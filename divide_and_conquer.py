...
split_islands = join(workspace, 'split_islands.shp')
viewsheds = join(workspace, 'viewshed_polys2.shp')
grid = join(workspace, 'select_grid.shp')

viewshed_layer = arcpy.MakeFeatureLayer_management(viewsheds, 'viewshed_lyr')

# for i, (pg,) in enumerate(SearchCursor(grid, ['SHAPE@'])):
#     arcpy.SelectLayerByLocation_management(
#         in_layer=viewshed_layer,
#         overlap_type="INTERSECT",
#         select_features=pg,
#         selection_type="NEW_SELECTION",
#     )
#     print(arcpy.GetCount_management(viewshed_layer)[0])
#     if i == 1:
#         print(get_field_names(viewshed_layer))
#         for (s, poly) in SearchCursor(viewshed_layer, ['FID_split_', 'SHAPE@']):
#             print('{}: {}'.format(s, poly.area))

spatial_reference = arcpy.Describe(join(workspace, 'split_islands.shp')).spatialReference
